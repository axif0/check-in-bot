...
async function run() {
  try {
    ...
    const searchResponse = await octokit.paginate(
    ...
    for (const item of searchResponse) {
      ...
      core.info(
        `\n[${processedCount}/${searchResponse.length}] ${itemType} #${
          item.number
        }: "${item.title || "No title"}"`
      );
      core.info(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
      ...
      const stopCommentObj = comments.find(
        (c) =>
          ...
          c.body?.toLowerCase().includes(stopComment.toLowerCase())
      );
      ...
      if (hasStopComment) {
        stopCommentCount++;
        const stopCommentDate = new Date(
          stopCommentObj.created_at
        ).toISOString();
        const stopCommentUser = stopCommentObj.user?.login || "unknown user";

        if (autoAddLabel) {
          core.info(
            `📛 SKIPPING: Found stop-comment '${stopComment}' from ${stopCommentUser} on ${stopCommentDate}`
          );
          ...
          core.info(
            `✓ Label '${ignoreLabel}' applied to ${itemType} #${item.number}`
          );
        } else {
          core.info(
            `📛 SKIPPING: Found stop-comment '${stopComment}' from ${stopCommentUser} on ${stopCommentDate}`
      ...
      let lastUserActivity =
        userComments.length > 0
          ? new Date(
              Math.max(
                ...userComments.map((c) => new Date(c.created_at).getTime())
              )
            )
      ...
      const lastUserName =
        userComments.length > 0
          ? userComments.find(
              (c) =>
                new Date(c.created_at).getTime() === lastUserActivity.getTime()
            )?.user?.login || "unknown"
          : item.user?.login || "unknown";

      core.info(
        `📅 Last user activity: ${lastUserActivity.toISOString()} by ${lastUserName}`
      );
      ...
      const lastBotActivity =
        botComments.length > 0
          ? new Date(
              Math.max(
                ...botComments.map((c) => new Date(c.created_at).getTime())
              )
            )
          : null;
      ...
      const daysSinceLastUser =
        (now.getTime() - lastUserActivity.getTime()) / (1000 * 60 * 60 * 24);
      ...
      core.info(
        `${
          inactivityCondition ? "✓" : "✗"
        } Inactivity threshold met (${daysSinceLastUser.toFixed(
          2
        )} days >= ${daysInactive} days required)`
      );
      core.info(
        `${
          botActivityCondition ? "✓" : "✗"
        } Bot hasn't commented or user commented after bot`
      );
      ...
      if (shouldComment) {
        ...
        let finalMessage = checkInMessage;
        ...
        const daysInactiveVars = [
          "{{ days-inactive }}",
          "{{days-inactive}}",
          "{{ days-inactive}}",
          "{{days-inactive }}",
        ];
        const stopCommentVars = [
          "{{ stop-comment }}",
          "{{stop-comment}}",
          "{{ stop-comment}}",
          "{{stop-comment }}",
        ...
        for (const daysInactiveVar of daysInactiveVars) {
          if (finalMessage.includes(daysInactiveVar)) {
            finalMessage = finalMessage
              .split(daysInactiveVar)
              .join(daysInactive.toString());
            break;
          }
        }
        ...
        if (!inactivityCondition) {
          core.info(
            `  ∟ Reason: Not inactive long enough (${daysSinceLastUser.toFixed(
              2
            )} days < ${daysInactive} days required)`
          );
        }
        if (!botActivityCondition) {
          core.info(
            `  ∟ Reason: Bot already commented after the last user activity`
          );
        }
        skippedCount++;
      }
